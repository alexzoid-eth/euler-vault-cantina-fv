...
- Shares cannot be deposited, minted or skimmed to the zero address (`deposit`, `mint`, `skim`)
- The user's balance and total vault's shares are increased when shares are deposited, minted or skimmed (`deposit`, `mint`, `skim`)
- Any user's balance is always less than or equal to the total vault's shares plus accumulated fees
- If the balance forwarder is enabled, the balance tracker hook is called when user shares are changed (`deposit`, `mint`, `skim`, `withdraw`, `redeem`)
- Balance forwarder is never executed when the balance forwarder is not enabled (`deposit`, `mint`, `skim`, `withdraw`, `redeem`)
- Balance forwarder can be executed only with specific functions (`deposit`, `mint`, `skim`, `withdraw`, `redeem`)
- The balance tracker hook is called with `forfeitRecentReward` set to `false` when user's balance in increased (`deposit`, `mint`, `skim`)
- The balance tracker hook is called with `forfeitRecentReward` set to `true` only when collateral control is in progress (`withdraw`, `redeem`)
...
- Excess assets available to skim are calculated as the vault's balance minus the vault's cash (`skim`)
- Max `uint256` requested to skim assets means using the entire excess vault's balance (`skim`)
- Skimming more assets than the excess vault's internal balance is not allowed (`skim`)
- Operating with zero assets or zero shares is not allowed (`skim`)
- Skimmed assets are converted to shares, rounded down (`skim`)
- Skimmed assets increase the vault's cash balance (`skim`)
...
- Zero withdraw amount do nothing and return zero (`withdraw`)
- Withdraw asset amounts are converted to shares, rounding up (`withdraw`)
- Withdraw and redeem are reverted if the vault's cash is insufficient (`withdraw`, `redeem`)
- Allowance is not decreased if the amount is zero or if the owner is the spender (`withdraw`, `redeem`)
- A `uint256` maximum allowance means unlimited allowance, so it is not decreased during operations (`withdraw`, `redeem`)
- The operation reverts if the allowance is less than the required amount (`withdraw`, `redeem`)
- The user's allowance for the spender is updated after being decreased (`withdraw`, `redeem`)
- Withdraw and redeem could not be processed if the allowance is insufficient (`withdraw`, `redeem`)
- Withdraw and redeem reverts if the user's original balance is less than the required amount (`withdraw`, `redeem`)
- The user's balance and total vault's shares are decreased when shares are withdrawn or redeemed (`withdraw`, `redeem`)
- Zero redeem amount do nothing and return zero (`redeem`)
- Max `uint256` shares redeem withdraw using all user's balance assets (`redeem`)
- Zero assets cannot be redeemed from non-zero shares (`redeem`)
- Withdraw and redeem transfer assets from the vault to the receiver (`withdraw`, `redeem`)
- Withdraw and redeem check `transfer` result while transferring to the receiver (`withdraw`, `redeem`)
- Withdraw and redeem decrease the vault's cash amount (`withdraw`, `redeem`)
- Assets cannot be withdrawn to the zero address (`withdraw`, `redeem`)
- Assets cannot be withdrawn or redeemed to sub-accounts when asset itself is NOT compatible with EVC (`withdraw`, `redeem`)